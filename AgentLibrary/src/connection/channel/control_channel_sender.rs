...
impl ControlChannelSender {
    ...
    pub async fn disconnect(&mut self) {
        match self.stop_signal_tx.take() {
            Some(stop_signal) => {
                ...
                Logger::append_system_log(LogLevel::INFO, "Control Channel: Destroyed Sender successfully.".to_string()).await;
            },
            None => Logger::append_system_log(LogLevel::ERROR, "Control Channel: Failed to destroy Sender.".to_string()).await,
        }
    }
    ...
    pub async fn send<T: Packet + Send + 'static>(&mut self, packet: T) {
        ...
        if let Err(err) = self.sender_tx.send(packet) {
            Logger::append_system_log(LogLevel::ERROR, format!("Control Channel: Unable to submit packet to Send Thread.\nReason: {}", err)).await;
        }
    }
}
